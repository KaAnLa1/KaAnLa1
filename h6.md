...
- Tor seeks to frustrate attackers from linking communication partners, or from
...
- Components are 
	- Onion Proxy (OP) which is local software intalled on the user's device. It
...
(e.g., websites, blogs, other hidden services). If users want to access a HS, they need to find its onion address through these
platforms.
